...
class PosEscposController(http.Controller):
    ...
    def _get_header(self, p, company):
        ...
        p.set(align="center")
        if company.phone:
            p.textln(_("Tel: %s") % company.phone)
        if company.email:
    ...
    def _get_order_info(self, p, order, LINE_WIDTH):
        p.textln("" * LINE_WIDTH)
        p.set(align="center")
        p.textln(_("Served by %s") % order.user_id.name)
        p.textln(order.name)
    ...
    def _get_order_lines(self, p, order, currency, LINE_WIDTH):
        for line in order.lines:
            ...
            qty_measure = line.product_id.uom_id.name or _("unit(s)")
        ...
        spaces = LINE_WIDTH - len(_("TOTAL")) - len(amount_total)
        p.set(custom_size=True, width=1, height=2)
        p.textln(
            _("%(total)s%(spaces)s%(amount)s")
            % {
                "total": _("TOTAL"),
                "spaces": " " * spaces,
                "amount": amount_total,
    ...
    def _get_tax_detail(self, p, order, currency, LINE_WIDTH):
        ...
        for tax_name, amounts in tax_summary.items():
            ...
            column_titles = [_("Tax"), _("Base"), _("Tax Amount")]
